...
document.addEventListener('DOMContentLoaded', () => {
                ...
                card.innerHTML = `
                    <div class="person-item-left">
                        ...
                        <div class="person-info-col">
                            <span class="person-full-name">${person.firstName} ${person.lastName}</span>
        ...
        if (editId) {
            ...
            if (target) {
                ...
                if (target.photo && personPhotoPreview) {
                    ...
                    personPhotoPreview.innerHTML = `<img src="${target.photo}" alt="Photo" style="width:100%; height:100%; object-fit:cover; border-radius:8px;">`;
                }
            }
        }
    ...
    if (personPhotoUploadBox && personPhotoInput) {
        personPhotoUploadBox.addEventListener('click', () => {
        ...
        personPhotoInput.addEventListener('change', (e) => {
            ...
            if (file && personPhotoPreview) {
                ...
                reader.onload = (event) => {
                    ...
                    personPhotoPreview.innerHTML = `<img src="${event.target.result}" alt="Photo" style="width:100%; height:100%; object-fit:cover; border-radius:8px;">`;
                    personPhotoPreview.style.border = 'none';
                };
                reader.readAsDataURL(file);
            }
    ...
    // ----------------------------------------------------------------------
    let skillsData = [
        { id: 1, name: 'CNC Taşlama', score: '5/10' },
        { id: 2, name: 'CNC Dişli', score: '7/10' }
    ];
    ...
    const skillOpSelect = document.getElementById('skillOpSelect');
    ...
    function openSkillModal(editId = null) {
        ...
        if (editId) {
            ...
            if (target) {
                if (skillOpSelect) skillOpSelect.value = target.name;
                if (skillScoreSelect) skillScoreSelect.value = target.score;
            }
        } else {
            if (skillOpSelect) skillOpSelect.value = '';
    ...
    if (saveSkillModalBtn) {
        saveSkillModalBtn.addEventListener('click', () => {
            const opName = skillOpSelect ? skillOpSelect.value : '';
    ...
    let achievementsData = [
        { id: 1, title: 'Başarı Adı', approvedBy: 'Approved by Çağatay Cangüloğlu' },
        { id: 2, title: 'Başarı Adı', approvedBy: 'Approved by Çağatay Cangüloğlu' },
        { id: 3, title: 'Başarı Adı', approvedBy: 'Approved by Çağatay Cangüloğlu' }
    ];
    ...
    function renderInternalOps(filterQuery = '') {
        ...
        internalOpCountBadge.textContent = `${internalOpsData.length} Operasyon`;
                ...
                card.innerHTML = `
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <div style="display: flex; flex-direction: column;">
                            ...
                            <span style="font-size: 12px; color: #667085;">${op.type || 'İç Operasyon'}</span>
                        </div>
                    </div>
                    <div class="dept-item-actions">
    ...
    function renderOpManagers(filterQuery = '') {
        ...
        filtered.forEach(m => {
            ...
            card.innerHTML = `
                <input type="checkbox" style="accent-color: #7F56D9; width: 18px; height: 18px; cursor: pointer;">
                <div class="person-avatar-wrapper" style="width: 36px; height: 36px; min-width: 36px; min-height: 36px;">
    ...
    let suppliersData = [
        { id: 1, name: 'Tedarikçi', checked: true },
        { id: 2, name: 'Tedarikçi', checked: true },
        { id: 3, name: 'Tedarikçi', checked: true },
        { id: 4, name: 'Tedarikçi', checked: true },
        { id: 5, name: 'Tedarikçi', checked: true },
        { id: 6, name: 'Tedarikçi', checked: true }
    ];
        ...
        suppliersData.forEach((s) => {
            ...
            itemRow.innerHTML = `
                <div class="supplier-item-left">
                    <input type="checkbox" class="supplier-checkbox" ${s.checked ? 'checked' : ''} data-id="${s.id}">
                    <span>${s.name}</span>
                </div>
                    ...
                    card.innerHTML = `
                        <div style="display: flex; align-items: center; gap: 12px;">
                            ...
                            <div>
                                ...
                                <div style="font-size: 12px; color: #667085; margin-top: 2px;">Dış Operasyon</div>
                            </div>
                        </div>
                        <div class="dept-item-actions">
    ...
    function renderExtOpManagers(filterQuery = '') {
        ...
        listToRender.forEach(m => {
            ...
            item.innerHTML = `
                <input type="checkbox" class="supplier-checkbox" data-id="${m.id}">
                <div>
    ...
    if (extOpManagerSearchInput) {
        extOpManagerSearchInput.addEventListener('input', (e) => {
            renderExtOpManagers(e.target.value);
        });
    }
    ...
    function openEquipmentModal(id = null) {
        ...
        if (id !== null && id !== undefined) {
            ...
            if (eqIdInput) eqIdInput.value = Math.floor(1000000000 + Math.random() * 9000000000).toString();
        ...
        saveEquipmentModalBtn.addEventListener('click', () => {
            ...
            if (editingEquipmentId !== null && editingEquipmentId !== undefined) {
                ...
                if (target) {
                    ...
                    if (currentUploadedEqPhoto) target.img = currentUploadedEqPhoto;
                }
            } else {
                equipmentsData.push({
                    id: Date.now(),
        ...
        if (shiftsData.length === 0) {
            ...
            if (definedShiftsList) {
                ...
                shiftsData.forEach(item => {
                    ...
                    const card = document.createElement('div');
                    card.style.cssText = `padding: 14px 16px; border: 1px solid #EAECF0; border-left: 4px solid ${borderColor}; border-radius: 8px; background: #FFFFFF; display: flex; align-items: center; justify-content: space-between;`;
                    card.innerHTML = `
                        <div>
                            <div style="font-size: 15px; font-weight: 700; color: #101828;">${item.name}</div>
                            <div style="font-size: 13px; color: #667085; margin-top: 2px;">${item.startTime} - ${item.endTime}</div>
                        </div>
                        <div class="dept-item-actions">
    ...
    function switchShiftTab(tabName) {
        ...
        if (tabName === 'info') {
            if (shiftTabInfo) shiftTabInfo.style.display = 'block';
            if (shiftTabBreak) shiftTabBreak.style.display = 'none';
            if (shiftTabInfoBtn) {
                shiftTabInfoBtn.style.borderBottomColor = '#7F56D9';
                shiftTabInfoBtn.style.color = '#7F56D9';
                shiftTabInfoBtn.style.fontWeight = '600';
            }
            if (shiftTabBreakBtn) {
                shiftTabBreakBtn.style.borderBottomColor = 'transparent';
                shiftTabBreakBtn.style.color = '#667085';
                shiftTabBreakBtn.style.fontWeight = '500';
            }
        } else if (tabName === 'break') {
            if (shiftTabInfo) shiftTabInfo.style.display = 'none';
            if (shiftTabBreak) shiftTabBreak.style.display = 'block';
            if (shiftTabBreakBtn) {
                shiftTabBreakBtn.style.borderBottomColor = '#7F56D9';
                shiftTabBreakBtn.style.color = '#7F56D9';
                shiftTabBreakBtn.style.fontWeight = '600';
            }
            if (shiftTabInfoBtn) {
                shiftTabInfoBtn.style.borderBottomColor = 'transparent';
                shiftTabInfoBtn.style.color = '#667085';
                shiftTabInfoBtn.style.fontWeight = '500';
            }
        }
    }
    ...
    function openShiftModal(id = null) {
        ...
        if (id !== null && id !== undefined) {
            ...
            if (target) {
                ...
                const colorRadio = document.querySelector(`input[name="shiftCalendarColor"][value="${target.color || 'kırmızı'}"]`);
            ...
            const defaultColorRadio = document.querySelector('input[name="shiftCalendarColor"][value="kırmızı"]');
    ...
    if (saveShiftModalBtn) {
        saveShiftModalBtn.addEventListener('click', () => {
            ...
            const color = colorRadio ? colorRadio.value : 'kırmızı';
